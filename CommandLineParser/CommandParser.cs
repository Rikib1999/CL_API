...
namespace CommandLineParser
{
    ...
    public static class CommandParser<T> where T : ICommandDefinition
    {
        ...
        public static T Parse(string command, T commandInstance, bool beginsWithName = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(newCommand)) throw new CommandParserException("Abundant options, arguments or parameters found.");
        ...
        {
            if (option.Delimeter == '-') throw new CommandParserException("Delimeter of option " + option.Names[0] + " can not be '-'.");
            if (option.Delimeter == '-') throw new CommandParserException("Delimeter of option " + option.Names[0] + " can not be '.'.");
        }
        ...
        private static GeneralType CheckType(PropertyInfo property, out Type propType, out Type coreType)
        {
            ...
            if (propType.IsEnum) generalType = GeneralType.Enum;
            if (propType.IsArray) generalType = GeneralType.Array;
            if (propType.IsGenericType && (propType.GetGenericTypeDefinition() == typeof(List<>))) generalType = GeneralType.List;
            ...
            if (option == null || option.MaxParameterCount > 0)
            {
                ...
                if (generalType == GeneralType.Enum) isSupported = true;
                ...
                {
                    if (typeArgs[0].GetConstructor(new Type[] { typeof(string) }) != null || typeArgs[0].IsEnum) isSupported = true;
                    coreType = typeArgs[0];
                }
                ...
                if (!isSupported)
                {
                    if (option != null) throw new CommandParserException("Option " + option.Names[0] + " can not be parsed. Type of option [" + propType.Name + "] is not supported.");
                    if (argument != null) throw new CommandParserException("Argument " + argument.Name + " can not be parsed. Type of argument [" + propType.Name + "] is not supported.");
                }
            }
        ...
        {
            if (string.IsNullOrEmpty(argument.Name)) throw new CommandParserException("Argument name can not be empty.");
        ...
        private static string FindParameters(string command, Option option, string usedName, int startIndex, out List<string> parameters)
        {
            ...
            while (parameters.Count < option.MaxParameterCount && currentPosition < command.Length)
            {
                ...
                {
                    if (command.Length == currentPosition + 1 || command[currentPosition + 1] < '0' || command[currentPosition + 1] > '9') break;
                }
            ...
            if (parameters.Count < option.MinParameterCount && !minEmptyStr) throw new CommandParserException("Option " + option.Names[0] + " has insufficient number of parameters.");
        ...
        private static List<(string, int)> FindArgumentParameters(ref string command, int argsDelimeterIndex, bool findMultiple)
        {
            ...
            if (string.IsNullOrEmpty(command)) return paramsAndIndices;
            ...
            {
                if (!command.Contains(' ') || command.IndexOf(' ', currentPosition + 1) == -1) return paramsAndIndices;
                currentPosition = command.IndexOf(' ', currentPosition + 1) + 1;
            }

            command = command.Substring(currentPosition).Trim();
            if (argsDelimeterIndex != int.MaxValue) argsDelimeterIndex -= currentPosition;
            currentPosition = 0;
            ...
            while (currentPosition < command.Length)
            {
                ...
                {
                    if (!ignoreDash && argsDelimeterIndex != int.MaxValue) command = command.Remove(argsDelimeterIndex).Trim();
                }
        ...
        private static int PopulateCommandInstance(ref T commandInstance, PropertyInfo property, GeneralType generalType, Type coreType, List<string> parameters, bool isOption = true)
        {
            ...
            if (coreType is not IConvertible && coreType.GetConstructor(new Type[] { typeof(string) }) != null) castMethod = typeof(ValueCaster).GetMethod("CastClass");
        ...
        private static T ParseArguments(T commandInstance, CommandReferenceBag commandReferenceBag, int argsDelimeterIndex, ref string command)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                if (a != null) argsAndProps.Add((a, property));
            }
            ...
            foreach ((Argument, PropertyInfo) argAndProp in argsAndProps)
            {
                ...
                if (paramsAndIndices.Count == 0) break;
                ...
                if (indexOfExcParam == 0 && argAndProp.Item1.IsRequired) throw new CommandParserException("Required argument " + argAndProp.Item1.Name + " is missing or can not be parsed.");
                ...
                if (indexOfExcParam >= 0)
                {
                    ...
                    if (argsDelimeterIndex != int.MaxValue) argsDelimeterIndex -= paramsAndIndices[indexOfExcParam].Item2;
                }
                else if (generalType == GeneralType.Simple || generalType == GeneralType.Enum)
                {
                    ...
                    if (argsDelimeterIndex != int.MaxValue) argsDelimeterIndex -= paramsAndIndices[0].Item2 + +paramsAndIndices[0].Item1.Length;
                }
                else
                {
                    ...
                    if (argsDelimeterIndex != int.MaxValue) argsDelimeterIndex -= paramsAndIndices[paramsAndIndices.Count - 1].Item2 + +paramsAndIndices[paramsAndIndices.Count - 1].Item1.Length;
                }
            ...
            foreach ((Argument, PropertyInfo) argAndProp in argsAndProps)
            {
                if (argAndProp.Item1.IsRequired && commandReferenceBag.ArgsPresence[argAndProp.Item1.Name] == false) throw new CommandParserException("Required argument " + argAndProp.Item1.Name + " is missing.");
            }
